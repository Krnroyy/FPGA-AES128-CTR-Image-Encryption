...
// Version 1 supports a 96-bit IV, no AAD, and arbitrary payload length through
// TKEEP/TLAST. GHASH is serialized between blocks to preserve authentication
// state and downstream backpressure is fully honored.
module AES_GCM_AXIS #
(
...
localparam [1:0] AUTH_WAIT_DATA   = 2'd1;
localparam [1:0] AUTH_WAIT_LENGTH = 2'd2;
...
wire [31:0] control_write_value = apply_wstrb(32'd0, write_data,
                                               write_strb);
wire [31:0] mode_write_value = apply_wstrb({31'd0, decrypt_mode},
    ...
    begin
        ...
        begin
            case(write_addr[6:0])
                ...
                REG_MODE:
                    decrypt_mode <= mode_write_value[0];
...
wire [127:0] authenticated_ciphertext = mask_standard_bytes(
    decrypt_mode ? input_hold : payload_result, keep_hold);
...
begin
    ...
    begin
        ...
        begin
            ...
            if(ghash_done)
            begin
                if(auth_state == AUTH_WAIT_DATA)
                begin
                    ghash_state <= ghash_result;
                    if(auth_last)
                    begin
                        ghash_x <= ghash_result ^
                                   {64'd0, final_message_bits};
                        ghash_h <= hash_subkey;
                        ghash_start <= 1'b1;
...
begin
    case(s_axi_ctrl_araddr[6:0])
        ...
        REG_STATUS: read_value = {
            24'd0, auth_fail, tag_match, tag_valid, gcm_ready,
            stream_enable, output_valid, stream_done, stream_busy
        };
        REG_BLOCKS: read_value = block_count;
        REG_MODE: read_value = {31'd0, decrypt_mode};
