...
static int receive_batch_count(u32 *count)
{
    static const u8 expected_magic[4] = {'G', 'B', 'H', '1'};
    u8 header[8];
    u32 index;

    receive_exact(header, 8U);
    for(index = 0U; index < 4U; ++index)
        if(header[index] != expected_magic[index])
            return XST_FAILURE;
    *count = pack_be32(&header[4]);
    if(*count == 0U || *count > MAX_BATCH_IMAGES)
        return XST_FAILURE;
    return XST_SUCCESS;
}
...
static int compare_tags(const u32 left[4], const u32 right[4])
{
    u32 index;
    for(index = 0U; index < 4U; ++index)
        if(left[index] != right[index])
...
static void print_tag(u32 image_index, const u32 tag[4])
{
    xil_printf("GCM_TAG %u %08x%08x%08x%08x\r\n",
...
int main(void)
{
    ...
    init_platform();
    xil_printf("\r\nZCU104_AES_GCM_DMA_BATCH_START\r\n");
    ...
    xil_printf("READY_FOR_BATCH_COUNT %u\r\n",
               (unsigned int)MAX_BATCH_IMAGES);
    if(receive_batch_count(&batch_count) != XST_SUCCESS)
    {
        xil_printf("ERROR INVALID_BATCH_HEADER\r\n");
        cleanup_platform();
        return 2;
    }
    xil_printf("BATCH_COUNT %u\r\n", (unsigned int)batch_count);

    for(image_index = 0U; image_index < batch_count; ++image_index)
    {
        xil_printf("READY_FOR_PACKET %u %u\r\n",
                   (unsigned int)image_index,
                   (unsigned int)PACKET_SIZE_BYTES);
        if(receive_image_packet() != XST_SUCCESS)
        {
            xil_printf("ERROR INVALID_PACKET_MAGIC %u\r\n",
                       (unsigned int)image_index);
            cleanup_platform();
            return 3;
        }
        xil_printf("IMAGE_RECEIVED %u %u\r\n",
                   (unsigned int)image_index,
                   (unsigned int)IMAGE_SIZE_BYTES);
        xil_printf("IV %u %08x%08x%08x\r\n", (unsigned int)image_index,
                   (unsigned int)initial_iv[0],
                   (unsigned int)initial_iv[1],
                   (unsigned int)initial_iv[2]);

        xil_printf("ENCRYPTING_GCM_DMA %u\r\n",
                   (unsigned int)image_index);
        ...
        if(dma_process_gcm(input_image, encrypted_image, MODE_ENCRYPT, NULL,
           ...
           (final_status & STATUS_TAG_MATCH) == 0U)
        {
            xil_printf("ERROR GCM_DMA_ENCRYPT %u\r\n",
                       (unsigned int)image_index);
            cleanup_platform();
            return 4;
        }
        ...
        print_tag(image_index, encryption_tag);

        xil_printf("DECRYPTING_AUTHENTICATED_GCM_DMA %u\r\n",
                   (unsigned int)image_index);
        start_count = read_arm_counter();
        ...
        {
            xil_printf("ERROR GCM_DMA_DECRYPT %u\r\n",
                       (unsigned int)image_index);
            cleanup_platform();
            return 5;
        }
        ...
        if((final_status & (STATUS_TAG_MATCH | STATUS_AUTH_FAIL)) !=
           STATUS_TAG_MATCH || !compare_tags(encryption_tag, check_tag) ||
           mismatch != 0)
        {
            xil_printf("ERROR VALID_AUTH_OR_RECOVERY %u %d\r\n",
                       (unsigned int)image_index,
                       mismatch == 0 ? -1 : mismatch - 1);
            cleanup_platform();
            return 6;
        }
        xil_printf("AUTHENTICATION_PASS %u\r\n",
                   (unsigned int)image_index);
        xil_printf("RECOVERY_PASS %u\r\n",
                   (unsigned int)image_index);

        xil_printf("CIPHERTEXT_BEGIN %u %u\r\n",
                   (unsigned int)image_index,
                   (unsigned int)IMAGE_SIZE_BYTES);
        send_bytes(encrypted_image, IMAGE_SIZE_BYTES);
        xil_printf("\r\nCIPHERTEXT_END %u\r\n",
                   (unsigned int)image_index);
        xil_printf("RECOVERED_BEGIN %u %u\r\n",
                   (unsigned int)image_index,
                   (unsigned int)IMAGE_SIZE_BYTES);
        ...
        xil_printf("TAMPER_BYTE_INDEX %u %u\r\n",
                   (unsigned int)image_index,
                   (unsigned int)TAMPER_BYTE_INDEX);
        xil_printf("DECRYPTING_TAMPERED_GCM_DMA %u\r\n",
                   (unsigned int)image_index);
        start_count = read_arm_counter();
        if(dma_process_gcm(encrypted_image, recovered_image, MODE_DECRYPT,
                           encryption_tag, check_tag,
                           &final_status) != XST_SUCCESS)
        {
            xil_printf("ERROR GCM_DMA_TAMPER_OPERATION %u\r\n",
                       (unsigned int)image_index);
            cleanup_platform();
            return 7;
        }
        end_count = read_arm_counter();
        print_performance("TAMPERED_DECRYPTION_GCM_DMA",
                          start_count, end_count);
        if((final_status & (STATUS_TAG_MATCH | STATUS_AUTH_FAIL)) !=
           STATUS_AUTH_FAIL || compare_tags(encryption_tag, check_tag))
        {
            xil_printf("ERROR TAMPER_NOT_REJECTED %u\r\n",
                       (unsigned int)image_index);
            cleanup_platform();
            return 8;
        }
        xil_printf("AUTHENTICATION_FAIL_EXPECTED PASS %u\r\n",
                   (unsigned int)image_index);
        xil_printf("TAMPER_REJECTED %u\r\n",
                   (unsigned int)image_index);
        xil_printf("PLAINTEXT_RELEASED NO %u\r\n",
                   (unsigned int)image_index);
        zeroize_buffer(recovered_image, IMAGE_SIZE_BYTES);
        if(!buffer_is_zero(recovered_image, IMAGE_SIZE_BYTES))
        {
            xil_printf("ERROR ZEROIZATION_FAILED %u\r\n",
                       (unsigned int)image_index);
            cleanup_platform();
            return 9;
        }
        xil_printf("TAMPERED_BUFFER_ZEROIZED %u\r\n",
                   (unsigned int)image_index);
        xil_printf("ZCU104_AES_GCM_DMA_IMAGE_DONE %u\r\n",
                   (unsigned int)image_index);
    }

    xil_printf("ZCU104_AES_GCM_DMA_BATCH_DONE %u\r\n",
               (unsigned int)batch_count);
    cleanup_platform();
    return 0;
}
